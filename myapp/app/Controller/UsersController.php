...
App::uses('CakeEmail', 'Network/Email');
  ...
  public $components = array('Session', 'Email', 'Auth' => array(
		        'authenticate' => array(
		            'Form' => array(
		                'userModel' => 'User',
						 'scope' => array('deleted'=>0,'status'=>1),
		                'fields' => array(
		                    'username' => 'email',
		                    'password' => 'password'
		                )
...
public function login(){
	 ...
	 if (!empty($this->request->data)) {
	    ...
	    if ($this->Auth->login()) {
					...
					$this->redirect('/trades/accounts');
		  }
	 		else{
	 			$this->Session->setFlash('Login failed','default',array('class'=>'error1'));
	 		}
	 	}
}
...
public function signup(){
	...
	if(!empty($this->request->data)){
		...
		$capture_data = $data['g-recaptcha-response'];
	 	if(!empty($capture_data)){
	 		//your site secret key
	        $secret = '6LcP3EsUAAAAACOj2t3mWJXKk7BTesKKb05jlgvP';
	        //get verify response data
	        $verifyResponse = file_get_contents('https://www.google.com/recaptcha/api/siteverify?secret='.$secret.'&response='.$capture_data);
	        ...
	        if($responseData->success){
				...
				if($pass == $cpass){
					...
					if($this->User->validates()){
						if($this->User->save($data)){
							...
							$hash_url = 'https://clickmegh.com/cryptofolio/users/activate/'.$hash;
							$to = $data['User']['email'];
							$subject = 'Account activation';
							$message = 'Welcome to CryptoFolio auto trading. To activate your account, we need to verify your E-mail address. If you ('.$to.') have requested the creation of this account, please activate your account by clicking the link below.'.$hash_url;
							$this->User->sendActivationEmail($to,$subject,$message,$hash);
							//$this->User->SendMailText($to,$subject,$message);
	 		...
	 		$this->Session->setFlash('Please check the security box','default',array('class'=>'error1'));
	 	}
	}
}
...
public  function reset_password(){
	...
	if(!empty($this->request->data)){
		...
		$search_user = $this->User->find('count', array('conditions'=>array('deleted'=>0,'status_id'=>1,'email'=>$email)));

	  	if($search_user > 0){
			if($this->User->updateAll(array('password'=>$pw),array('deleted'=>0,'status_id'=>1,'email'=>$email))){
				$to = $data['User']['email'];
				$subject = 'Password Reset';
				$message = 'Your password has been reset and your temporal password is => '.$default_pwd.' <= Please change your password when you first login.';
				$this->User->sendGeneralEmail($to,$subject,$message);
			
				$this->Session->setFlash('Your password has been reset. Check your inbox for the new password');
			}
		}
		else{
